...
- **M23** — Rework funcional de bases y ambientación Fantasy Village. Paquete ambiental **Low Poly Fantasy Village Environment** (OccaSoftware) en `Assets/OccaSoftware/Low Poly Fantasy Village/` (URP, 60+ prefabs, escena demo solo como referencia); auditado en `Docs/M23_FANTASY_VILLAGE_ASSET_AUDIT.md`. No se usa Buto ni niebla volumétrica: se conserva la fog of war propia. Los assets originales del paquete no se modifican; el arte derivado vive en `Assets/CierzoArena/Art/Environment/FantasyVillage/` como prefab/material variants. El builder (`Cierzo Arena → Create MOBA Greybox Arena`) genera cada base como aldea fortificada: núcleo lógico (`CoreGameplayRoot`) separado del town center visual, spawn del héroe **detrás** del núcleo, zona de tienda junto al spawn con `ShopkeeperAnchor` (el paquete no incluye vendedor; la tecla `B` sigue funcionando), tres gateways con la 3.ª torre de cada carril defendiéndolos, y **dos Core Guard Towers** delante del núcleo, espejadas Azure/Ember. Regla de vulnerabilidad (fuente única de verdad autoritativa en `StructureProgressionController`): perder las tres torres de cualquier carril → `BaseBreached` → ambas Core Guards atacables; destruir ambas Core Guards → `CoreVulnerable` → núcleo atacable → victoria. Regenerar la escena reproduce el mismo resultado. Tras el rework hay que **rehornear el NavMesh** en editor. Para sustituir un edificio o añadir props, edita una prefab variant propia (nunca el original) y colócala en la rama `Visuals` de la base, manteniendo los props decorativos sin `Targetable`, sin `Team` y con colliders simples o ninguno para no romper NavMesh ni la separación gameplay/visual. Detalles en `Docs/M23_BASE_REWORK.md`. Pruebas: Edit Mode (`StructureAndMatchTests`); Host/Client y Play Mode pendientes de validación en editor.
- **M23 (builders)** — La paleta y el layout se pueblan con `Cierzo Arena → Environment → Build Fantasy Village Palette` (assets en `Assets/CierzoArena/Settings/Environment/`). `Create MOBA Greybox Arena` los usa automáticamente para generar las aldeas, la ambientación de mapa (`FantasyVillageMapAmbienceBuilder`) y las rutas interiores de creeps. Valida el resultado con `Cierzo Arena → Environment → Validate M23 Base Rework`. El NavMesh se construye en runtime (capa Ground) al entrar en Play; no hay bake de editor. Para sustituir un edificio, edita la paleta (referencia a otra variant) y regenera; para añadir props, amplía `FantasyVillageBaseBuilder`/`FantasyVillageMapAmbienceBuilder` manteniéndolos en capa default sin colliders de gameplay para no romper NavMesh.
...
18. Para probar M17, mantén `Tab`: muestra las filas Azure y Ember con nivel, K/D/A, last hits, oro, XP, daño y objetivos. Mata un héroe con apoyo de otro para comprobar kill/assist; compra un objeto para ver que baja el oro actual sin bajar el oro ganado. Al destruir un núcleo, el marcador se congela y muestra Victoria o Derrota para el héroe local.
